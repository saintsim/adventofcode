...
def find_solution(zero_node_id):
    ...
    for i in range(3001):
        if i == 1000 or i == 2000 or i == 3000:
            print(i, " -> ", NODES[next_ptr].val)
            total += NODES[next_ptr].val
        next_ptr = get_next(next_ptr)
    return total


def populate_nodes(input, decryption_key):
    j = 0
    ...
    for token in input:
        ...
        NODES[j] = Node(int(token)*decryption_key, prev_ptr, next_ptr)
        prev_ptr = j
        if int(token) == 0:
            zero_node_id = j
        j += 1
...
def grove(input, mix_times, decryption_key):
    zero_node_id = populate_nodes(input, decryption_key)
    for _ in range(mix_times):
        for i in range(len(input)):
            ...
            if node_to_move.val < 0:
                ...
                if node_to_move_new_prev == node_to_move.prev_ptr:
                    continue
                ...
                if node_to_move_new_next == node_to_move.next_ptr:
                    continue
                # update the old neighbours of the moved one
                NODES[get_node_ptr(node_to_move_old_prev)].next_ptr = node_to_move_old_next
                NODES[get_node_ptr(node_to_move_old_next)].prev_ptr = node_to_move_old_prev
...
if __name__ == '__main__':
    with open('input', 'r') as file:
        ...
        print('Result: ' + str(grove(lines, 1, 1)))